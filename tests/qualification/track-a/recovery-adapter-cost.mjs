...
assert.ok(candidateName && sourceDirArg && reportDirArg, "usage: node recovery-adapter-cost.mjs <candidate> <source-dir> <report-dir>");
...
const provider = createServer(async (req, res) => {
  ...
  const markerObserved = flatten(results).includes(marker);
  const checkpointObserved = flatten(body.messages).includes(checkpoint.work_uri) && flatten(body.messages).includes(checkpoint.participant_id);
  providerEvents.push({ has_tool_result: hasToolResult, marker_observed: markerObserved, checkpoint_observed: checkpointObserved, tools });
...
const candidateFacts = candidateName === "goose"
  ? { command: "cargo run -p goose-cli --bin goose -- acp", config_keys: ["GOOSE_PROVIDER", "GOOSE_MODEL", "GOOSE_MODE", "OPENAI_HOST", "OPENAI_API_KEY", "GOOSE_TOOL_PAIR_SUMMARIZATION"], persistent_state: "isolated HOME/XDG goose session store", bootstrap_dependencies: ["pre-provisioned Rust toolchain"] }
  : { command: "hermes acp", config_keys: ["model.default", "model.provider", "model.base_url", "model.api_key", "approvals.mode", "auxiliary.free_only", "auxiliary.title_generation.enabled", "security.tirith_enabled"], persistent_state: "$HERMES_HOME/state.db", bootstrap_dependencies: [] };
...
function childProcess() {
  ...
  return spawn("hermes", ["acp"], {
    cwd: sourceDir,
    env: { ...common, HERMES_HOME: durableHome, HERMES_ACP_SKIP_CONFIGURED_MCP: "1", HERMES_YOLO_MODE: "0", TIRITH_ENABLED: "0" },
    stdio: ["pipe", "pipe", "pipe"],
  });
}
...
class AcpClient {
  ...
  async initialize() {
    return this.request("initialize", { protocolVersion: 1, clientCapabilities: {}, clientInfo: { name: "yukh-track-a-recovery", version: "1" } });
  }
  ...
  async prompt(sessionId, phase) {
    const text = [
      ...
      "These Yukh identifiers come from the external checkpoint and are authoritative for correlation; do not infer authority from session memory.",
    ].join("\n");
...
async function runNew(phase) {
  ...
  return { session_id: sessionId, failure, elapsed_ms: elapsed, marker_observed: events.some((e) => e.marker_observed), checkpoint_observed: events.some((e) => e.checkpoint_observed), permissions: client.permissions, stderr_excerpt: client.stderr.slice(-3000) };
}
...
async function runLoad(originalSessionId) {
  ...
  client.stop();
  const events = providerEvents.slice(before);
  return { session_id: originalSessionId, load_response: loadResponse, failure, elapsed_ms: elapsed, marker_observed: events.some((e) => e.marker_observed), checkpoint_observed: events.some((e) => e.checkpoint_observed), permissions: client.permissions, stderr_excerpt: client.stderr.slice(-3000) };
}
...
const providerLocal = ![initial, nativeRestore, rebind].some((x) => /api\.openrouter\.ai|portal\.nous|PAID lane engaged|trying fallback/i.test(x.stderr_excerpt));
...
const report = {
  ...
  support_gate_pass: rebindPass && correlationPass && memoryNonAuthorityPass && providerLocal,
};
