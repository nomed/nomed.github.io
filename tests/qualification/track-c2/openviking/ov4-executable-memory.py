...
def find_memory(client: SyncHTTPClient, memory_root: str):
    ...
    result = client.find(query="service transport HTTP JSON", target_uri=memory_root, limit=10, options={"context_type": ["memory"], "read_content": True})
    candidates = []
...
def host_a(server_url: str, output: str) -> int:
    ...
    memory_policy = {"self": {"enabled": True}, "peer": {"enabled": False}, "working_memory": {"enabled": False}, "memory_types": ["preferences"]}
    session_info = client.create_session(session_id=session_id, options={"memory_policy": memory_policy, "auto_commit_policy": None})
    memory_root = canonical_memory_root(session_info)
    session = client.session(session_id=session_id)
    session.add_message(role="user", content="Remember this project preference: service transport is HTTP/JSON.")
    session.add_message(role="assistant", content="Recorded as contextual memory only.")
    ...
    payload = {"stage": "host-a", "pid": os.getpid(), "candidate_revision": CANDIDATE_REVISION, "session_id": session_id, "session_uri": session_info.get("uri"), "memory_root": memory_root, "task_id": task_id, "task_status": task.get("status"), "memory_uri": memory_uri, "memory_content": memory_content, "memory_sha256": digest(memory_content), "remembered_claim_present": REMEMBERED_TEXT in memory_content, "find_total": find_result.get("total") if isinstance(find_result, dict) else None, "public_seams": ["create_session", "add_message", "commit", "get_task", "find", "tree", "read"]}
...
def host_b(server_url: str, stage_a_path: str, output: str) -> int:
    ...
    payload = {"stage": "host-b-after-restart", "pid": os.getpid(), "candidate_revision": CANDIDATE_REVISION, "memory_uri": stage_a["memory_uri"], "memory_content": memory_content, "memory_sha256": digest(memory_content), "same_memory_as_host_a": digest(memory_content) == stage_a["memory_sha256"], "remembered_claim_present": REMEMBERED_TEXT in memory_content, "public_seams": ["initialize", "read"]}
    write_json(output, payload)
    client.close()
    return 0
    ...
    try:
        ...
        if client is not None:
            try:
                ...
                pass
    payload = {"stage": "provider-outage", "pid": os.getpid(), "candidate_revision": CANDIDATE_REVISION, "provider_unavailable": provider_unavailable, "provider_error_class": error_class, "provider_probe": "public list_sessions GET /api/v1/sessions", "accepted_fact": accepted, "accepted_fact_sha256": digest(accepted_text), "accepted_fact_available_without_provider": accepted.get("value") == "gRPC/protobuf"}
    write_json(output, payload)
...
def finalize(stage_a_path: str, stage_b_path: str, outage_path: str, provider_log_path: str, output: str) -> int:
    ...
    observations = {
        "host_processes_distinct": a["pid"] != b["pid"], "commit_completed": a["task_status"] == "completed",
        "canonical_memory_root_used": a.get("memory_root", "").startswith("viking://user/") and "~/" not in a.get("memory_root", ""),
        "remembered_claim_persisted": a["remembered_claim_present"], "cross_host_reuse": b["same_memory_as_host_a"],
        ...
        "memory_classification": "remembered", "accepted_fact_classification": accepted.get("class"),
        "memory_is_accepted_state": False, "memory_is_evidence_truth": False,
        ...
        "bounded_vlm_calls": chat_calls >= 1, "bounded_embedding_calls": embedding_calls >= 1,
    }
    required = ("host_processes_distinct", "commit_completed", "canonical_memory_root_used", "remembered_claim_persisted", "cross_host_reuse", "restart_persistence", "accepted_fact_conflicts_with_memory", "accepted_fact_remains_authoritative", "outage_correctness", "bounded_vlm_calls", "bounded_embedding_calls")
    passed = all(observations[key] for key in required) and observations["memory_is_accepted_state"] is False and observations["memory_is_evidence_truth"] is False
    report = {
        "schema_version": 1, "track": "C2", "gate": "OV-4-executable-memory-profile", "candidate": "volcengine/OpenViking", "revision": CANDIDATE_REVISION,
        "profile": {
            "public_surface": "openviking-sdk HTTP", "auth_mode": "dev",
            "model_path": "documented OpenAI-compatible VLM + embedding endpoints",
            "model_endpoint": "loopback-only 127.0.0.1:21935/v1",
            "external_model_egress_configured": False,
            "external_model_egress_basis": "all configured VLM/embedding endpoints are loopback and production provider credentials are blank; general process egress was not instrumented or observed",
        ...
        "provider_memory": {"uri": a["memory_uri"], "sha256": a["memory_sha256"], "classification": "remembered", "claim": "service transport is HTTP/JSON"},
        "accepted_fact": {"fact_id": accepted.get("fact_id"), "value": accepted.get("value"), "class": accepted.get("class"), "authority": accepted.get("authority"), "sha256": o["accepted_fact_sha256"]},
        "model_traffic": {"chat_completion_calls": chat_calls, "embedding_calls": embedding_calls, "request_paths": paths},
        "observations": observations,
        "outage_evidence": {"probe": o.get("provider_probe"), "error_class": o.get("provider_error_class"), "provider_unavailable": o.get("provider_unavailable")},
